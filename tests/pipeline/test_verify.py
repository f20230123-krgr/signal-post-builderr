...
def _fact(field_name, value, source_url="https://www.equinor.com/about", extraction_method="structured", content_hash="abc123"):
    return RawFact(
...
def test_threshold_boundary_case_is_explicit(monkeypatch):
    monkeypatch.setattr(verify_module, "_name_similarity", lambda a, b: float(NAME_MATCH_THRESHOLD))
    accepted = verify(_fact("organization_name", "anything"), _entity())
    assert accepted is not None
    assert accepted.match_confidence == NAME_MATCH_THRESHOLD

    monkeypatch.setattr(verify_module, "_name_similarity", lambda a, b: NAME_MATCH_THRESHOLD - 0.01)
